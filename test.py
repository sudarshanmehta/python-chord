                ...
                try:
                    ...
                    if tries == 5:
                        ...
                        print("Expected : '%s', got '%s'" % (
                            data[key], peers[random.randrange(len(peers))].get(key)))
                        raise e
        else:
...
# create addresses
address_list = list(map(lambda addr: Address('127.0.0.1', addr), list(set(map(lambda x: random.randrange(40000, 50000),
                                                                               range(10))))))
# keep unique ones
...
for i in range(0, len(address_list)):
    ...
    except socket.error:  # socket busy
